...
proc msg_prot {unick uhost hand text} {
   ...
   if {$unick != $owner} {
      if {[info exists twice_msg($unick)]} {
         ...
         utimer 10 { putlog "NO REPLY" }
         if {[string match "*freenode*" $uhost]} {
...
proc ban_chk {nick uhost handle channel mchange bhost} {
   ...
   if {[string match [string tolower $bhost] [string tolower $botname]]} {
      if {![matchattr $nick f] && $nick != $botnick && $nick != "ChanServ" && ![string match "*freenode*" $nick] && ![info exists igflood($nick)]} {
         if {[matchattr $cflag D]} {
            if {$quick == "1"} {
...
proc prot:deop {nick uhost handle channel mchange {opnick ""}} {
   ...
   if {$mode == "-o"} {
      ...
      if {$opnick == $botnick} {
         ...
         if {![matchattr $nick f] && $nick != "ChanServ" && ![string match "*freenode*" $nick] && ![string match "*Guest*" $botnick]} {
...
proc sl_ctcp {nick uhost hand dest key arg} {
   ...
   if {[string match "*freenode*" [string tolower $uhost]]} {
...
putlog "============================="
putlog " $vern LoadEd"
putlog "Reported any bugs to chat.freenode.net ##0dev"
putlog "http://0dev.us.to"
putlog "============================="
